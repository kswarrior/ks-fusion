...
> Current: `v2.3` (78/100 in docs/vs.md) — tree-walk interpreter in Go, gradual-typed `.ks` language
> (`: type` annotations, `is`, `?.`/`??`, `ok`/`err` results, 158 builtins, --bin, fmt/vet/doc/check/repl/bench).
> This doc lists what exists, what is planned, and what is explicitly non-goals.

## 1. Where we are (v2.3, real today)
...
* Types (v2.3, gradual): `nil bool int float number string array map func chan any ok err` —
...
* Stdlib (158 builtins): all v2.1 plus `http_get/post/fetch_json/http_serve`,
...
* Tooling (v2.3): `fusion fmt/vet/doc/check/repl/bench` + `test`, constant folding.
* Build (v2.3): `fusion build --bin` single static binary + `--target` matrix + `fusion.lock` semver + `vendor/`.
* Frontend (v2.3): `fusion run-web` SSR + `fusion build-js` per-route JS + budgets.
...
* [x] `select` for channels (with `timeout` / `default`). Done v2.1:
  ...
  Done v2.3: `fusion run --race` + `with_timeout`/`parallel`.
* [x] `fusion fmt` (canonical formatter) + `fusion vet` (unused var, bad arity lint). Done v2.3:
...
* [x] `fusion doc` from `#` comments. Done v2.3: `fusion doc [target] [--out FILE]`.
* [ ] Namespaced imports: `import "hello-lib" as hl` / `hl.greet()`.
  Today: flat globals, prefix your functions.
* [x] `fusion.lock` + real semver resolver (`^0.1.0`, `>=`, path + git deps). Done v2.3:
  `^ ~ >= > < *` + `fusion.lock` + `fusion vendor`. Done v2.3: registry `publish/pull/yank` + sha256 + namespaces + private token. Left: git deps, audit.
...
* [x] `fusion build --bin` → single static executable (embed `.ks`+`.kslib`,
  like `go build`). Done v2.3: `fusion build --bin [-o FILE] [--target OS/ARCH]`, verified 11M isolated run.
* [x] Cross-compile: `--target linux/amd64,arm64,darwin,windows,wasm`. Done v2.3 (GOOS/GOARCH passthrough).
...
* [x] Tree-walk opts done v2.1 (Perf 4→5): lock-free single-threaded scopes
  ...
  Done v2.3 (Perf 5→6): constant folding (`1+2`→`3`, `"a"+"b"`→`"ab"`, `frontend/fold.go`, idempotent, tested).
