# Missing List — v2.3 done (score 78/100 → 78–82 next)
...
> v2.3: 50 → 78 (+19: Perf+1 Types+1 Concurrency+1 Stdlib+4 Ecosystem+2 Tooling+3 Build+3 Frontend+2 Maturity+2).
...
- [x] `fusion version`, `run/build/launch --help`, v2.3 single constant
...
- [x] `fusion doc` from `#` comments (done v2.3)
- [x] `fusion check` strict (parse + arity + `: type` + `is` narrowing) (done v2.3)
- [ ] Namespaced imports (`import "x" as h`) — today flat globals
- [x] `fusion.lock` + semver resolver (`^`, `~`, `>=`, path) — done v2.3 (git deps left)
...
- [x] `--target` matrix (linux/amd64,arm64,darwin,windows,wasm via GOOS/GOARCH) (done v2.3)
- [ ] `--cpuprofile`, opcode counts
- [x] Constant folding (done v2.3); [ ] local-slot lookup, string-builder concat
- [x] `fusion run --race`, `with_timeout`/`parallel` (done v2.3); [ ] deterministic scheduler

## Language core P1 (Types 6→7 done)

- [x] Gradual `: type`, `is`/`is not`, `?.`/`??`, `ok`/`err` + helpers + `struct_validate/enum_create` (done v2.3)
...
- [x] Regex `regex_*` builtins (done v2.3); [ ] Regex literal
- [ ] `defer` inside `go` routines

## Stdlib P1 (Stdlib 4→8 done)

- [x] 158 builtins (v2.1 97 + v2.3 52: http/regex/crypto/fs/process/time/db/log/concurrency/types)
...
- [x] P1: `fusion check` + `vet` frontend rules (done v2.3)
- [x] P2: `fusion run-web` SSR (HTML+JSON, `/api/*`) (done v2.3, HMR left)
- [ ] P3: client hydrate + `use_state`/`on_mount` (stub done, full left) + `fetch_json` (done)
- [x] P4: `fusion build-js` per-route + manifest + budgets (done v2.3)
...
- [x] `fusion vendor` offline builds (done v2.3); [ ] private registries + token auth; `scope/name` namespaces

## DX P2 (Tooling/Maturity)

- [x] REPL (`fusion repl` history + multiline) (done v2.3)
- [ ] LSP + VS Code extension (hover, goto-def, rename, format-on-save)
- [ ] Debugger (`--debug`, breakpoints, stack)
- [x] Benchmarks (`fusion bench`) (done v2.3, criterion-style reports left)
